...
export const BUILT_IN_OPENWORK_EXTENSION_MANIFESTS: OpenWorkExtensionManifest[] = [
  ...
  },
  {
    ...
    id: "univer-cli",
    name: "Univer CLI",
    description: "Built-in Univer cowork bundle for native .univer spreadsheets, documents, and slides.",
    preview: true,
    source: { format: "openwork-builtin", origin: "builtin", trusted: true },
    icon: { src: "/openwork-mark.svg" },
    composer: { prompt: UNIVER_CLI_COMPOSER_PROMPT },
    setup: {
      instructions: "OpenWork includes a matched Univer CLI runtime, cowork UI package, and canonical univer-cli skill bundle. OpenWork verifies the bundled executable, skill resources, managed inspect tools, and SaC migration templates before marking setup ready.",
      primaryCta: "Repair Univer bundle",
      secondaryCta: "Check setup",
      testActionRef: "openwork.univerCli.setupStatus",
    },
    resources: [
      {
        type: "skill",
        ...
        description: "Canonical skill package pinned by the @univer/cowork bundle, including references and inspect tools.",
        path: "builtin://univer-cli",
        required: true,
      },
      {
        type: "native-binary",
        id: "univer-bundled-executable",
        label: "Bundled univer executable",
        packageName: "@univer/cowork",
        required: true,
      },
      {
        type: "tool",
        id: "univer-setup-status",
        label: "Setup status",
        required: true,
      },
      {
        type: "tool",
        id: "univer-setup-install",
        label: "Bundle repair",
        required: true,
      },
      {
        type: "tool",
        id: "univer-health-checks",
        label: "Bundle health checks",
        description: "Executable help/version, inspect tool listing, and SaC migration template listing.",
        required: true,
      },
    ],
    contributions: [
    ...
    lifecycle: {
      ...
      detection: ["skill:univer-cli", "binary:univer"],
    },
  },
  {
    schemaVersion: 1,
