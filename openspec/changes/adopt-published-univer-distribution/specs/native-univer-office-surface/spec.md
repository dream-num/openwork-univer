...
OpenWork SHALL route `.univer` spreadsheet, document, and slide navigation, viewing, worktree review, merge preview, and supported editing through the Published Cowork SDK's Cowork Content Viewer component from `@univerjs-pro/cowork`, backed by the collab gateway service, rather than iframe-embedding the gateway-served collab-client page or using OpenWork's generic artifact editor.

#### Scenario: User opens a native target

- **WHEN** a user opens a `.univer` artifact from OpenWork
- **THEN** OpenWork SHALL display it with the `Cowork Content Viewer` component from `@univerjs-pro/cowork`
...
- **THEN** OpenWork SHALL explicitly import `@univerjs-pro/cowork/viewer/styles.css`
